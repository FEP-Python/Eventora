...
export const Dashboard = () => {
    ...
    const { data: analytics } = useGetBudgetAnalytics(Number(orgId));
    const router = useRouter();
    const [isMounted, setIsMounted] = useState(false);

    const { activeOrg } = useOrgStore();
    const { isAuthenticated } = useIsAuthenticated();
    const { data } = useOrgDashboard(Number(orgId));
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 w-full">
                                <div className="flex items-center justify-between w-full gap-6">
                                    <div className="rounded-xl bg-blue-50 py-4 px-10 flex flex-col items-center w-full">
                                        <p className="text-xs text-blue-800 font-medium flex items-center gap-2 mb-2">
                                    ...
                                    <div className="rounded-xl bg-green-50 py-4 px-10 flex flex-col items-center w-full">
                                        <p className="text-xs text-green-800 font-medium flex items-center gap-2 mb-2">
                                    ...
                                    <div className="rounded-xl bg-fuchsia-50 py-4 px-10 flex flex-col items-center w-full">
                                        <p className="text-xs text-fuchsia-800 font-medium mb-2">Utilization</p>
