...
const createOrganization = async (orgData: CreateOrgRequest) => {
    const response = await axios.post("http://localhost:5000/api/org/create", orgData, {
...
const updateOrganization = async ({ id, ...updateData }: UpdateOrgRequest & { id: number }) => {
    const res = await axios.patch(`http://localhost:5000/api/org/update/${id}`, updateData, {
        headers: getAuthHeaders()
    });
    return {
...
const deleteOrganization = async (id: number) => {
    const res = await axios.delete<ApiResponse>(`http://localhost:5000/api/org/delete/${id}`, {
        headers: getAuthHeaders(),
    });
